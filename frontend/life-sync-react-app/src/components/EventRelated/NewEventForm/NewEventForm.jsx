...
const NewEventForm = (props) => {
    ...
    useEffect(() => {
        const validateForm = !Object.values(newEvent).some((item) => item === null || item.length === 0,);
        if (validateForm) {
    ...
    const handleDescription = (value) => setNewEvent({...newEvent, description: value});
    const handleLocation = (value) => setNewEvent({...newEvent, location: value});
    const handleNotification = (value) => setNewEvent({...newEvent, notification: value});
    ...
    const selectStartDate = (value) => {
        ...
        setSortTime({
            ...sortTime, startingTime: {...sortTime.startingTime, date: formattedValue},
        });
    ...
    const selectEndDate = (value) => {
        ...
        setSortTime({
            ...sortTime, endingTime: {...sortTime.endingTime, date: formattedValue},
        });
    ...
    const submitStartDate = (event) => {
        ...
        if (sortTime.endingTime.date < CalendarStartDate) {
            setSortTime({
                ...sortTime, endingTime: {...sortTime.endingTime, date: CalendarStartDate},
            });
            let formatted = format(CalendarStartDate, 'EEE. MMM. d, y');
    ...
    const submitEndDate = (event) => {
        ...
        if (sortTime.startingTime.date > CalendarEndDate) {
            setSortTime({
                ...sortTime, startingTime: {...sortTime.startingTime, date: CalendarEndDate},
            });
            let formatted = format(CalendarEndDate, 'EEE. MMM. d, y');
            setDisplayStartDate(formatted);
        }
